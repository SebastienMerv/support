...
<body>
    <form class="w-screen bg-slate-100 h-screen flex items-center flex-col" method="POST" action="{{ route('password.email') }}">
        @csrf
        ...
        <div class="bg-white w-80 flex flex-col items-center p-12 w-[800px] h-[400px]">
            ...
            <div class="flex flex-col w-96 m-8">
                ...
                <input id="email" name="email" type="email" placeholder="Votre adresse email" class="p-2 border border-gray-400 rounded-lg h-12">
            </div>

            <button type="submit" class="pt-2 pb-2 w-96 mt-4 rounded-lg bg-yellow-400 font-mono flex items-center justify-center font-semibold hover:bg-yellow-500">Envoyer le lien de réinitialisation</button>
        </div>
    </form>
</body>
</html>
